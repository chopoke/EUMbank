...
/* ===================== 적금 상세 컴포넌트 ===================== */
function InstallmentHistoryPage() {
  const { i_no } = useParams();
  ...
  React.useEffect(() => {
    ...
    fetchInstallmentDetail(Number(i_no))
      .then((res) => {
        const d = res.data;

        // 👉 이 부분은 백엔드 DTO 필드에 맞게 조정
        // 예시: i_account_no, i_currency, i_amount, i_join_date, i_rate ...
        const cur = (d.i_currency ?? d.currency ?? "KRW").toUpperCase();

        setAccount({
          bank: "EumBank",
          number: d.i_account_no || d.a_account_no || "",
          type: "적금",
          currency: cur,
  ...
  React.useEffect(() => {
    if (i_no) localStorage.setItem("last_i_no", String(i_no));
  }, [i_no]);
  ...
  React.useEffect(() => {
    ...
    fetchInstallmentTransactions(Number(i_no), {
      type,
      ...
      .then((res) => {
        ...
        const mapped = (p.content || []).map((r) => {
          ...
          const base = Number(
            r.th_amount ?? r.th_account_in ?? r.th_account_out ?? 0
          );
  ...
  }, [i_no, kinds, dateFrom, dateTo, page, pageSize, memoQv, sortKey, account?.currency]);
  ...
  const displayFilter = React.useMemo(() => {
    ...
    out = out.filter((r) => {
      ...
      if (memoQuery.trim()) {
        ...
        if (!(`${r.memo}${r.counterparty}`.toLowerCase().includes(q)))
          return false;
      }
      return true;
    });
  ...
  function downloadCsv(list) {
    ...
    a.href = url;
    a.download = "installment_history.csv";
    a.click();
  ...
  async function downloadPdf(list) {
    ...
    doc.save(`InstallmentHistory-${Date.now()}.pdf`);
  }
            ...
            <div className="rounded-2xl border bg-white p-5 shadow-sm flex items-center justify-between">
              <div>
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <span>·</span>
                  <div className="text-sm text-gray-600">적금 계좌</div>
                </div>
