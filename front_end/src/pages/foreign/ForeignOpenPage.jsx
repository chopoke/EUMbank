...
function toYYYYMMDD(v) {
  if (!v) return "";
  // 문자열: "yyyy-MM-dd" or "yyyyMMdd" or 기타
  if (typeof v === "string") return v.replace(/\D+/g, "").slice(0, 8);
  // 배열: [yyyy, m, d]
  if (Array.isArray(v) && v.length >= 3) {
    const [y, m, d] = v;
    return `${y}${String(m).padStart(2, "0")}${String(d).padStart(2, "0")}`;
  }
  // 객체: {year, monthValue|month, dayOfMonth|day}
  if (typeof v === "object") {
    const y = v.year ?? v.y;
...
const init = {
  ...
  birth: "",       // YYYYMMDD
  pinNumber: "",   // 거래 PIN(6) - 서버 검증 성공 시 저장
...
export default function ForeignOpenPage() {
  ...
  // 1) 내 정보(이름/생일/영문이름/고객번호) 프리필
  useEffect(() => {
    (async () => {
      const { data: me } = await api.get("/api/foreign/me");

      // 다양한 키 대응
      const rawId =
        me?.customerNo ?? me?.cNo ?? me?.c_no ?? me?.customerId ?? me?.id;
      const num = Number(String(rawId ?? "").replace(/\D+/g, ""));

      const cNameKr =
        me?.c_name_kr ?? me?.cNameKr ?? me?.nameKr ?? me?.name ?? "";
      const cNameEn =
        me?.c_name_en ?? me?.cNameEn ?? me?.nameEn ?? me?.englishName ?? "";

      const rawBirth =
        me?.c_birth_dt ?? me?.birth ?? me?.birthDate ?? me?.cBirthDt ?? "";
      const birth = toYYYYMMDD(rawBirth);

      setForm((f) => ({
        ...f,
        customerId: Number.isNaN(num) ? "" : num,
        name: cNameKr || f.name,              // ← 이름 프리필
        birth: birth || f.birth,              // ← 생년월일 프리필
  ...
  const next = async () => {
    // Step2 → Step3 넘어갈 때 영문명 없던 사용자가 입력했다면 1회 서버 저장
    if (step === 2 && !form.englishNameLocked && form.englishName.trim()) {
      try {
        await api.patch("/api/foreign/open/name-en", {
          englishName: form.englishName.trim(),
        });
        setForm((f) => ({ ...f, englishNameLocked: true }));
      } catch {
        alert("영문 이름 저장에 실패했습니다. 다시 시도해 주세요.");
        return;
      }
    }
  ...
  const onSubmit = async () => {
    ...
    if (!form.name) return alert("이름이 비어 있습니다.");
    if (!(form.englishNameLocked || form.englishName)) return alert("영문 이름을 입력해 주세요.");
    if (!/^\d{8}$/.test(form.birth)) return alert("생년월일은 YYYYMMDD 형식입니다.");
    if (!/^\d{6}$/.test(form.pinNumber)) return alert("거래 PIN(6자리) 확인이 필요합니다.");
    ...
    try {
      const { data } = await api.post("/api/foreign/open", {
        ...
        pin: form.pin, // 계좌 비밀번호(6)
        pinNumber: Number(form.pinNumber), // 서버가 다시 대조 가능
...
function Step2KYC({ form, setForm, prev, next }) {
  ...
  /* PIN 모달 상태 (한 번만 입력 후 서버 대조) */
  const [pinOpen, setPinOpen] = useState(false);

  return (
    <>
      <p className="fx-muted">
        본인 정보는 DB에서 자동으로 채워지며, 영문 이름은 최초 1회만 입력할 수 있습니다.
        거래 PIN은 서버와 대조하여 확인합니다.
      </p>

      <label className="fx-field">
        <span>이름</span>
        <input value={form.name} readOnly placeholder="이름" />
      </label>
      ...
      <label className="fx-field">
        <span>생년월일</span>
        <input value={form.birth} readOnly placeholder="YYYYMMDD" />
      </label>

      {/* 거래 PIN 확인(서버 대조) : 한 번만 입력 */}
      <label className="fx-field">
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span className="fx-muted">
            {/^\d{6}$/.test(form.pinNumber) ? "확인완료: ●●●●●●" : "미확인"}
          </span>
        </div>
      </label>
      ...
      >
        <PinPadModal
          ...
          onSubmit={async (val) => {
            try {
              const { data } = await api.post("/api/foreign/open/pin-verify", { pin: val });
              if (!data?.ok) {
...
const fallbackTerms = [
  { key: "terms", title: "외화 예금 거래 기본 약관", required: true, body:
`제1조(목적)
