  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 모달에서 c.product 사용 가능하도록 보정
        const normDeposits = Array.isArray(d) ? d.map((x) => ({ ...x, product: x.product ?? x.depositProduct ?? {} })) : [];
        const normSavings  = Array.isArray(s) ? s.map((x) => ({ ...x, product: x.product ?? x.installmentProduct ?? {} })) : [];
        const normLoans    = Array.isArray(l) ? l.map((x) => ({ ...x, product: x.product ?? {} })) : [];

        setDeposits(normDeposits);
        setSavings(normSavings);
        setLoans(normLoans);
      } catch (e) {
